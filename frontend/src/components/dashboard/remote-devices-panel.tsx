        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
            ...
            <div className="space-y-2">
                {DEMO_DEVICES.map((device, idx) => (
                    <div
                        key={idx}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border ${device.status === "online"
                                ? "bg-emerald-50 border-emerald-200"
                                : "bg-amber-50 border-amber-200"
                            }`}
                    >
                        {/* Status Dot */}
                        <div className={`w-3 h-3 rounded-full shrink-0 ${device.status === "online" ? "bg-emerald-500" : "bg-amber-500"
                            }`}>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className={`text-[9px] ${device.status === "online" ? "text-emerald-600" : "text-amber-600"
                                }`}>
                        ...
                        {/* Value */}
                        <span className={`text-sm font-extrabold shrink-0 ${device.status === "online" ? "text-emerald-600" : "text-amber-600"
                            }`}>
                            {device.lastValue}
                        </span>
                    </div>
