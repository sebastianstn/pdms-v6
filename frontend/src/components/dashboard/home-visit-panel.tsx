...
export function HomeVisitPanel({ visits }: HomeVisitPanelProps) {
    // TODO: Nutze echte Visits statt Demo-Daten wenn API vollständig verdrahtet
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex flex-col">
            ...
            <div className="space-y-2 flex-1">
                {displayVisits.map((visit, idx) => (
                    <div
                        key={idx}
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${visit.status === "done"
                    ...
                    >
                        {/* Status Icon */}
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0 ${visit.status === "done"
                        ...
                        <div className="min-w-0 flex-1">
                            <p className={`text-[10px] font-semibold ${visit.status === "upcoming" ? "text-slate-500" : "text-slate-900"
                                }`}>
                                {visit.time} — {visit.patient}
                            </p>
                            <p className={`text-[9px] ${visit.status === "active"
